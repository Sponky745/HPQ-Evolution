...
class Connection {
  ...
  copy() {
    return new Connection(this.a, this.b, this.inNeuron, this.outNeuron, this.weight);
  }
...
class NeuralNetwork {
  constructor(numI, numO, labelsI = [], labelsO = []) {
    ...
    for (let i = 0; i < numI; i++) {
      this.inputs.push(new Neuron(-1, labelsI[i]));
    }
  ...
  copy() {
    ...
    for (let i = 0; i < nn.neurons.length; i++) {
      for (let j = 0; j < nn.neurons[i].connections.length; j++) {
        let conn = nn.neurons[i].connections[j];
        
        if (conn.b < Number.MAX_SAFE_INTEGER - this.outputs.length) {
          conn.outNeuron = nn.neurons[conn.b];
        } else {
          let neuron = null;
          for (let k = 0; k < this.outputs.length; k++) {
            if (this.outputs[k].id == conn.b) {
              neuron = this.outputs[k];
            }
          }
          conn.outNeuron = neuron;
        }
        nn.conns.push(conn);
      }
    }
